...
class Pagos(models.Model):
    ...
    fechaHoraPAGO  = models.DateTimeField(blank=True, null=True)  # Verificacion de Pago con resumen de transferencias o compras #Script PC
    fechaHoraSmsTicketsPagados = models.DateTimeField(blank=True, null=True) #Envio de URL con tickets de QR oficial #Script PC
...
class Tickets(models.Model):
    ...
    fechaHoraCambio = models.DateTimeField(blank=True, null=True) # Fecha generacion cada vez cambia pin
                                                                  # cambio igual a generacion pero con reinicio si se transfiere
    fechaHoraConfirmado = models.DateTimeField(blank=True, null=True)  # Hora en que se da click a boton Confirmar
    fechaHoraPago       = models.DateTimeField(blank=True, null=True)  # Hora en que se corrobora el pago con resumenes de banco

    pin         = models.TextField(blank=True, null=True) # pin cambia codigo de seguridad
    pinIntentos = models.PositiveIntegerField(default=0)  # intento de cambio de pin #cuando se cambia o accede se reinicia
